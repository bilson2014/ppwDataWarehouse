...
public final class EntityMapper {
	...
	public static <TEntity> ColumnMap getColumnMap(TEntity entity, String entityName, boolean isFilter, String... columnNames) {
	...
	private static <TEntity> ColumnMap getColumnMapByColumnNames(TEntity entity, MetaDataTable metaDataTable, boolean isFilter,
			String... columnNames) {
		...
		for (final String columnName : columnNames) {
			...
			if (isFilter && (!metaColumns.containsKey(colName) || isFilterColumn(metaColumn)))
				continue;
			ColumnProperty colProperty = new ColumnProperty(metaColumn.getValue(entity)
					, metaColumn.getAnnotation().sqlType());
			mapTable.put(metaColumn.getName(), colProperty);
		}

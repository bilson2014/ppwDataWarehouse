 ...
 // 初始化树
	$('#reportTree').tree({
		...
		onLoadSuccess : function(node, data) {
			...
			$('#_easyui_tree_1').find('.tree-hit').removeClass('tree-collapsed');
			$('#_easyui_tree_1').find('.tree-hit').addClass('tree-expanded');
			$("#reportTree li:eq(1)").find("div").click();
...
ReportQuery.previewInNewTab = function () {
    ...
    if (node.attributes.flag == 1) {
        ...
        var previewUrl = queryPageRootUrl + "view/uid/" + node.attributes.uid;
        $('#tabs').tabs('add', {
            id: node.id,
            title: title,
